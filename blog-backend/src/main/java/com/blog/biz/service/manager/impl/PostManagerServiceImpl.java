...
public class PostManagerServiceImpl implements PostManagerService, OverviewStatisticService {
    ...
    private List<PostResponse> toPostResponses(List<PostEntity> postEntities) {
        ...
        Map<Long, CategoryEntity> categoryMap = categoryCrudService
            .listByIds(postEntities.stream().map(PostEntity::getCategoryId).collect(Collectors.toList()))
            .stream()
            .collect(Collectors.toMap(CategoryEntity::getCategoryId, categoryEntity -> categoryEntity));

        Map<Long, List<TagEntity>> tagMap = postTagsRela(
                postEntities.stream().map(PostEntity::getPostId).collect(Collectors.toList()));
        return postEntities.stream().map(postEntity -> {
            ...
            Optional.ofNullable(tagMap.get(postEntity.getPostId())).ifPresent(list -> {
                List<TagResponse> tags = list.stream()
     ...
     * 查询文章和标签关系
     * @param postIds Long>
     * @return java.util.Map<java.lang.Long, java.util.List <
     * com.blog.biz.model.entity.TagEntity>>
     */
    private Map<Long, List<TagEntity>> postTagsRela(List<Long> postIds) {
        if (CollectionUtils.isEmpty(postIds)) {
            return new HashMap<>();
        }
        List<PostTagRelaEntity> postTagRelaEntities = postTagRelaCrudService.listByFields(PostTagRelaEntity::getPostId,
                postIds);
        ...
        Map<Long, TagEntity> tagMap = tagCrudService.listByIds(tagIds)
            .stream()
            .collect(Collectors.toMap(TagEntity::getTagId, Function.identity()));
        return postTagRelaEntities.stream()
            .collect(Collectors.groupingBy(PostTagRelaEntity::getPostId,
